    ...
    class Controller { 
      ...
      editCity(buttonSaveEditions) {
        ...
        if (newCityName == "") {
          ...
          this.model.editCityInList(buttonSaveEditions, newCityName);
          wrapper.classList.toggle("show");
        }
      }
    ...
    makeExchangeCourseRequest() {
      ...
      getNameOfTheCityToAdd(inputAdd) {
        fetch(`http://api.openweathermap.org/data/2.5/weather?q=${inputAdd}&appid=61aafb341bc35ca228d34b505939ae6e`)
        .then(response => response.json())
        .then(response => {
          if (response.cod === "404") {
      ...
      addCityToList(name, temp, clouds, windSpeed) {
        ...
        .then(response => response.json())
        .then(response => {
          this.view.renderListOfCities(response.cityName, response._id, temp, clouds, windSpeed);
        })
        .catch(err => {
          this.view.input.value = `Oops...You have mistake. Status${err.status}. Try again`;
          setTimeout(()=>{
      ...
      prepareForRender(arrOfCities) {
        arrOfCities.forEach(item => {
          fetch(`http://api.openweathermap.org/data/2.5/weather?q=${item.cityName}&appid=61aafb341bc35ca228d34b505939ae6e`)
          .then( response => response.json())
          .then(response => {
            const temp = parseInt(response.main.temp - 273.15);
            const clouds = response.weather[0].description;
      ...
      editCityInList(buttonSaveEditions, newCityName) {
        const newCityLi = buttonSaveEditions.closest("li");
        const newCityDiv = newCityLi.firstElementChild;
        const cityId = buttonSaveEditions.dataset.id;
        fetch(`http://api.openweathermap.org/data/2.5/weather?q=${newCityName}&appid=61aafb341bc35ca228d34b505939ae6e`)
        .then(response => response.json())
        .then(response => {
          if (response.cod === "404") {
            this.view.input.value = `Oops...${response.message}!`;
